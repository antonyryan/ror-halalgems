...
(function() {
  var codeAddress, extractLast, geocoder, map, split, view_upload_details;
  ...
  $(document).ready(function() {
    ...
    $(function() {
      var carouselNavigation, carouselStage;
      carouselStage = $('.carousel-stage').on('jcarousel:create jcarousel:reload', function() {
  ...
  $(document).ready(function() {
    return $("#neighborhood_ids").tokenInput("/neighborhoods/index.json", {
      crossDomain: false,
      queryParam: "term",
  ...
  $(document).ready(function() {
    return $('#city_name').autocomplete({
      ...
      select: function(event, ui) {
        if (ui.item) {
          return $('#listing_city_id').val(ui.item.id);
        }
      }
    });
  });
  ...
  $(document).ready(function() {
    return $(".cloudinary-fileupload").fileupload({
      ...
      fail: function(e, data) {
        return $(".status").text("Upload failed");
      }
    });
  });

  $(document).ready(function() {
    return $(".cloudinary-fileupload").off("fileuploadsend").on("fileuploadsend", function(e, data) {
      var fields, progressDiv, regexp, row, time;
      row = $('#photos');
      ...
      return data.context = progressDiv.prependTo(row.find(".thumbnail:last"));
    });
  });

  $(document).ready(function() {
    $(".cloudinary-fileupload").off("cloudinarydone").on("cloudinarydone", function(e, data) {
      var options, row, time, upload_info;
      row = $('#photos');
      ...
      $('<input/>').attr({
        type: "hidden",
        name: 'listing[property_photos_attributes][' + time + '][photo_url]'
      }).val(upload_info).appendTo(data.form);
    });
  });

  view_upload_details = function(upload) {
    var rows;
    rows = [];
  ...
  jQuery(function() {
    ...
    return $('#myModal').on('show.bs.modal', function(event) {
      var alertErr, c, selected;
      selected = [];
      c = 0;
      $('#listings_from input:checked').each(function() {
        var field, form;
      ...
      return $('#myModal').on('shown.bs.modal', function(event) {
        return $('#email').focus();
      });
    });
  });
  ...
  jQuery(function() {
    return $('#send').click(function() {
      ...
      formValid = true;
      $('input').each(function() {
        var formGroup, glyphicon;
        formGroup = $(this).parents('.form-group');
        glyphicon = formGroup.find('.form-control-feedback');
        if (this.checkValidity()) {
          formGroup.addClass('has-success').removeClass('has-error');
          return glyphicon.addClass('glyphicon-ok').removeClass('glyphicon-remove');
        } else {
  ...
  jQuery(function() {
    ...
    taxesCheckBox.change(function() {
      return $('#listing_taxes_amount').prop('disabled', !this.checked);
    });
    return $('#narrow_button').click(function() {
      $('#listings_from input:not(:checked)').each(function() {
        return $(this).parents('tr').hide();
      });
      return $('#listings_from input:checked').prop('checked', false);
    });
  });
...
(function() {
  jQuery(function() {
    var aticks, data, options, pieoptions, plot_el, tmpArr;
    plot_el = $("#plot");
    if (plot_el.length) {
      data = $("#plot").data("var");
      tmpArr = $.makeArray(data);
      aticks = $.map(tmpArr, function(val, i) {
        return [[val.data[0][0] + 0.5, val.label]];
      });
      options = {
        series: {
          bars: {
